...
test "Translate" {
    ...
    const point = Tuples.Point(-3, 4, 5);
    const result = translate.multiplyTuple(point);
    const expectedResult = Tuples.Point(2, 1, 7);
...
test "Translate inverse" {
    ...
    const expectedResult = Tuples.Point(-8, 7, 3);
    try expect(result.equals(expectedResult));
}

test "Translate vector" {
    const translate = Transforms.Translate(5, -3, 2);
    const vector = Tuples.Vector(-3, 4, 5);
    const result = translate.inverse().multiplyTuple(vector);
    const expectedResult = Tuples.Vector(-3, 4, 5);
    try expect(result.equals(expectedResult));
}

test "scale" {
    const scale = Transforms.Scale(2, 3, 4);
    const point = Tuples.Point(-4, 6, 8);
    const result = scale.multiplyTuple(point);
    const expectedResult = Tuples.Point(-8, 18, 32);
    try expect(result.equals(expectedResult));
}

test "scale vector" {
    const scale = Transforms.Scale(2, 3, 4);
    const vector = Tuples.Vector(-4, 6, 8);
    const result = scale.multiplyTuple(vector);
    const expectedResult = Tuples.Vector(-8, 18, 32);
    try expect(result.equals(expectedResult));
}
...
test "scale inverse" {
    ...
    const vector = Tuples.Vector(-4, 6, 8);
    const result = scale.multiplyTuple(vector);
    const expectedResult = Tuples.Vector(-2, 2, 2);
    try expect(result.equals(expectedResult));
}

test "rotate x" {
    const point = Tuples.Point(0, 1, 0);
    ...
    const p1r = Tuples.Point(0, math.sqrt(2.0) / 2.0, math.sqrt(2.0) / 2.0);
    const p2 = fullQuarter.multiplyTuple(point);
    const p2r = Tuples.Point(0, 0, 1);
...
test "rotate y" {
    const point = Tuples.Point(0, 0, 1);

    const halfQuarter = Transforms.RotateY(math.pi / 4.0);
    const fullQuarter = Transforms.RotateY(math.pi / 2.0);

    const p1 = halfQuarter.multiplyTuple(point);
    const p1r = Tuples.Point(math.sqrt(2.0) / 2.0, 0, math.sqrt(2.0) / 2.0);
    const p2 = fullQuarter.multiplyTuple(point);
    const p2r = Tuples.Point(1, 0, 0);
...
test "rotate z" {
    ...
    const p1 = halfQuarter.multiplyTuple(point);
    const p1r = Tuples.Point(-math.sqrt(2.0) / 2.0, math.sqrt(2.0) / 2.0, 0);
    const p2 = fullQuarter.multiplyTuple(point);
    const p2r = Tuples.Point(-1, 0, 0);
...
test "sheer x in proportion to z" {
    ...
    const result = transform.multiplyTuple(point);
    const expectedResult = Tuples.Point(6, 3, 4);
    try expect(result.equals(expectedResult));
}
...
test "sheer y in proportion to x" {
    ...
    const result = transform.multiplyTuple(point);
    const expectedResult = Tuples.Point(2, 5, 4);
    try expect(result.equals(expectedResult));
}
...
test "sheer y in proportion to z" {
    ...
    const result = transform.multiplyTuple(point);
    const expectedResult = Tuples.Point(2, 7, 4);
    try expect(result.equals(expectedResult));
}
...
test "sheer z in proportion to x" {
    ...
    const result = transform.multiplyTuple(point);
    const expectedResult = Tuples.Point(2, 3, 6);
    try expect(result.equals(expectedResult));
}
...
test "sheer z in proportion to y" {
    ...
    const result = transform.multiplyTuple(point);
    const expectedResult = Tuples.Point(2, 3, 7);
    try expect(result.equals(expectedResult));
}

test "tranformations in sequence" {
    const point = Tuples.Point(1, 0, 1);
    ...
    try expect(p2.equals(Tuples.Point(1, -1, 0)));

    const p3 = scale.multiplyTuple(p2);
    try expect(p3.equals(Tuples.Point(5, -5, 0)));

    const p4 = translate.multiplyTuple(p3);
    try expect(p4.equals(Tuples.Point(15, 0, 7)));
}
...
test "transformations in seqence (chained)" {
    ...
    try expect(result.equals(Tuples.Point(15, 0, 7)));

    const result2 = point.multiplyMatrix(rotate).multiplyMatrix(scale).multiplyMatrix(translate);
    try expect(result2.equals(Tuples.Point(15, 0, 7)));
}
