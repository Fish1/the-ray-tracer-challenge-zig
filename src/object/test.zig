...
test "sphere normal x" {
    const sphere = Objects.Sphere();
    const normal = sphere.normalAt(Tuples.Point(1, 0, 0));
    try expect(normal.equals(Tuples.Vector(1, 0, 0)));
}

test "sphere normal y" {
    const sphere = Objects.Sphere();
    const normal = sphere.normalAt(Tuples.Point(0, 1, 0));
    try expect(normal.equals(Tuples.Vector(0, 1, 0)));
}

test "sphere normal z" {
    const sphere = Objects.Sphere();
    const normal = sphere.normalAt(Tuples.Point(0, 0, 1));
    try expect(normal.equals(Tuples.Vector(0, 0, 1)));
}

test "sphere normal non-axial" {
    const sphere = Objects.Sphere();
    const normal = sphere.normalAt(Tuples.Point(sqrt(3.0) / 3.0, sqrt(3.0) / 3.0, sqrt(3.0) / 3.0));
    try expect(normal.equals(Tuples.Vector(sqrt(3.0) / 3.0, sqrt(3.0) / 3.0, sqrt(3.0) / 3.0)));
}
...
test "normal on translated sphere" {
    ...
    const normal = sphere.normalAt(Tuples.Point(0, 1.70711, -0.70711));
    try expect(normal.equals(Tuples.Vector(0, 0.70711, -0.70711)));
}

test "normal on rotate & scale sphere" {
    var sphere = Objects.Sphere();
    sphere.transform = Transforms.Scale(1, 0.5, 1).multiply(Transforms.RotateZ(pi / 5.0));
    const normal = sphere.normalAt(Tuples.Point(0, sqrt(2.0) / 2.0, -sqrt(2.0) / 2.0));
    try expect(normal.equals(Tuples.Vector(0, 0.97014, -0.24254)));
}
